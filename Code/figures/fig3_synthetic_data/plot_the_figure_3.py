...
# ------------------------------ Small helpers ------------------------------
def add_subfigure_label(subfig: mpl.figure.SubFigure, label: str, x: float = 0.0, y: float = 1.0, fontsize: int = 14) -> None:
...
def choose_index_for_trials(n_trials_array: np.ndarray, n_target: int) -> tuple[int, int]:
    """Return (idx, actual_n) where idx points to the entry equal/closest to n_target."""
    idxs = np.where(n_trials_array == n_target)[0]
    idx = int(idxs[0]) if idxs.size else int(np.argmin(np.abs(n_trials_array - n_target)))
...
def create_subfigure_A(
    ...
    sig_mean_S: np.ndarray, sig_std_S: np.ndarray,  # kept for API compatibility
    xi_true: np.ndarray,
    xi_mean_S: np.ndarray, xi_std_S: np.ndarray,
    ref_ticks=(5, 50),
    ...
    for idx, (d, k) in enumerate(dk_pairs):
        ...
        for s in series_order:
            ...
            ax.fill_between(
                n_trials_array, ylo, yhi,
                color=col * (s + 1) / S, alpha=0.30, linewidth=0
            )
...
def create_subfigure_B(
    subfig: mpl.figure.SubFigure, title_prefix: str,
    n_trials_array: np.ndarray,
    eps_mean_S: np.ndarray, rho_mean_S: np.ndarray,             # theory means [S, L, K]
    eps_emp_mean: np.ndarray, eps_emp_std: np.ndarray,          # empirical (dots+bars)
    rho_emp_mean: np.ndarray, rho_emp_std: np.ndarray,
    ref_ticks=(5, 50),
    eps_std_S: np.ndarray | None = None, rho_std_S: np.ndarray | None = None,     # [S, L, K]
    series_labels=("fit from 5", "fit from 10", "fit from 15"),
):
    ...
    for k in range(K):
        ...
        for s in series_order:
            if eps_std_S is not None:
                ...
                ax_eps.fill_between(
                    n_trials_array, ylo, yhi,
                    color=(s + 1) / S * col, alpha=0.5, linewidth=0
                )

        # empirical
        ax_eps.errorbar(
            n_trials_array, eps_emp_mean[:, k], eps_emp_std[:, k],
            linestyle='', capsize=3, color=col * 0.5, alpha=1, label=None
        )
    ...
    for k in range(K):
        ...
        ax_rho.errorbar(
            n_trials_array, rho_emp_mean[:, k], rho_emp_std[:, k],
            linestyle='', capsize=3, color=col * 0.5, alpha=1, label=None
        )
...
def create_subfigure_C(
    ...
    line_5 = LineString([
        (inferred_5[t, 0] / np.sqrt(epsilon_ref[0, 0]),
         inferred_5[t, 1] / np.sqrt(epsilon_ref[1, 0])) for t in range(T)
    ])
    descaled_5 = scale(line_5.buffer(1), xfact=np.sqrt(epsilon_ref[0, 0]), yfact=np.sqrt(epsilon_ref[1, 0]), origin=(0, 0))
    ...
    axL.plot(true_modes[:, 0], true_modes[:, 1], color="red", linewidth=2, linestyle="--")
    ...
    line_50 = LineString([
        (inferred_50[t, 0] / np.sqrt(epsilon_ref[0, 1]),
         inferred_50[t, 1] / np.sqrt(epsilon_ref[1, 1])) for t in range(T)
    ])
    descaled_50 = scale(line_50.buffer(1), xfact=np.sqrt(epsilon_ref[0, 1]), yfact=np.sqrt(epsilon_ref[1, 1]), origin=(0, 0))
    patch_50 = PathPatch(polygon_to_path(descaled_50), facecolor='grey', alpha=0.4, lw=0)
    axR.add_patch(patch_50)
    axR.scatter(inferred_50[:, 0], inferred_50[:, 1], c=np.arange(T), s=8)
    axR.plot(true_modes[:, 0], true_modes[:, 1], color="red", linewidth=2, linestyle="--")
    axR.set_title(f"{ref_trials[1]} trials", fontweight="bold", fontsize=10, pad=2)
    axR.set_xlabel("PC1")
    axR.set_ylabel("PC2")

    # unify limits
    xlims = [min(axs[0].get_xlim()[0], axs[1].get_xlim()[0]),
             max(axs[0].get_xlim()[1], axs[1].get_xlim()[1])]
    ylims = [min(axs[0].get_ylim()[0], axs[1].get_ylim()[0]),
...
def create_subfigure_D_hist(
    ...
    offsets = (np.arange(num_bars_per_group) - (num_bars_per_group - 1) / 2.0) * bar_width
    ...
    for s in range(S):
        h = ax.bar(
            ...
            alpha=0.95,
            label=series_labels[s] if s == 0 else None  # keep legend appearance identical
        )
        if s == 0:
            handles.append(h)
...
def load_all_inputs():
    ...
    return (n_trials_array, sqrt_mean_sigma_true, xi_true, sig_attempts_S, xi_attempts_S,
            eps_attempts_S, rho_attempts_S, rho_emp_per_neuron_attempts,
            eps_emp_mean, eps_emp_std, rho_emp_mean, rho_emp_std,
            bar_x_true, inferred_y_5, inferred_y_50, epsilon_ref)
...
def aggregate_theory(sig_attempts_S, xi_attempts_S, eps_attempts_S, rho_attempts_S):
    ...
    rho_meanN_attempts = nanmean_preserve_all_nan(rho_attempts_S, axis=3)  # (A, S, L, K)
    ...
    return (rho_pred_mean_S, rho_pred_std_S,
            sqrt_mean_sigma_theory_mean_S, sqrt_mean_sigma_theory_std_S,
            xi_theory_mean_S, xi_theory_std_S,
            eps_pred_mean_S, eps_pred_std_S)


# ------------------------------ Main figure ------------------------------
def main():
    (n_trials_array, sqrt_mean_sigma_true, xi_true, sig_attempts_S, xi_attempts_S,
     eps_attempts_S, rho_attempts_S, rho_emp_per_neuron_attempts,
     eps_emp_mean, eps_emp_std, rho_emp_mean, rho_emp_std,
     bar_x_true, inferred_y_5, inferred_y_50, epsilon_ref) = load_all_inputs()

    (rho_pred_mean_S, rho_pred_std_S,
     sqrt_mean_sigma_theory_mean_S, sqrt_mean_sigma_theory_std_S,
     xi_theory_mean_S, xi_theory_std_S,
     eps_pred_mean_S, eps_pred_std_S) = aggregate_theory(
        sig_attempts_S, xi_attempts_S, eps_attempts_S, rho_attempts_S
    )
    ...
    create_subfigure_B(
        subfig_B, "Regular", n_trials_array,
        eps_pred_mean_S, rho_pred_mean_S,   # (S, L, K) — rho already summed over N
        eps_emp_mean, eps_emp_std,
        rho_emp_mean, rho_emp_std,
        ref_ticks=ref_trials,
        eps_std_S=eps_pred_std_S, rho_std_S=rho_pred_std_S,
        series_labels=SERIES_LABELS
    )

    # C: trajectories with uncertainty ribbons based on epsilon_ref
    subfig_C = fig.add_subfigure(bottom_gs[0])
    create_subfigure_C(subfig_C, ref_trials, bar_x_true, inferred_y_5, inferred_y_50, epsilon_ref)
