...
OUT_DIRS = [
    Path("cached_results/1 Gallego-Carracedo et al/real_gc_grid")
]
IMAGE_PATHS = [
    Path("cached_results/1 Gallego-Carracedo et al/Subfig_A.png")
]
...
def load_saved(out_dir: Path):
    ...
    # --- slice to Kplot
    pred = {
        "trials" : {"mean_rho": safe_slice_K(pr_tr_rho, Kplot), "mean_rho_std": safe_slice_K(pr_tr_rho_std, Kplot),
                    "eps": safe_slice_K(pr_tr_eps, Kplot), "eps_std": safe_slice_K(pr_tr_eps_std, Kplot)},
        "neurons": {"mean_rho": safe_slice_K(pr_ne_rho, Kplot), "mean_rho_std": safe_slice_K(pr_ne_rho_std, Kplot),
                    "eps": safe_slice_K(pr_ne_eps, Kplot), "eps_std": safe_slice_K(pr_ne_eps_std, Kplot)},
        "animals": {"mean_rho": safe_slice_K(pr_an_rho, Kplot), "mean_rho_std": safe_slice_K(pr_an_rho_std, Kplot),
                    "eps": safe_slice_K(pr_an_eps, Kplot), "eps_std": safe_slice_K(pr_an_eps_std, Kplot)},
    }
    emp = {
        "trials" : {"mean_rho": safe_slice_K(em_tr_rho, Kplot), "mean_rho_std": safe_slice_K(em_tr_rho_std, Kplot),
                    "eps": safe_slice_K(em_tr_eps, Kplot), "eps_std": safe_slice_K(em_tr_eps_std, Kplot)},
        "neurons": {"mean_rho": safe_slice_K(em_ne_rho, Kplot), "mean_rho_std": safe_slice_K(em_ne_rho_std, Kplot),
                    "eps": safe_slice_K(em_ne_eps, Kplot), "eps_std": safe_slice_K(em_ne_eps_std, Kplot)},
        "animals": {"mean_rho": safe_slice_K(em_an_rho, Kplot), "mean_rho_std": safe_slice_K(em_an_rho_std, Kplot),
                    "eps": safe_slice_K(em_an_eps, Kplot), "eps_std": safe_slice_K(em_an_eps_std, Kplot)},
    }
    ...
    if meta is not None and "available_data" in meta:
        ...
        ref_neurons = int(len(meta["available_data"].get("pre_neuron_ids", [])) or int(neurons_grid[0]))
        ref_animals = int(meta["available_data"].get("D_pre", int(D_grid[0])))
    else:
        print("[WARN] Using grid first entries as reference markers (meta.json missing/invalid).")
        ref_trials, ref_neurons, ref_animals = int(trials_grid[0]), int(neurons_grid[0]), int(D_grid[0])
...
def _plot_axis_pair(ax_eps, ax_rho, x,
                    pred_eps, pred_eps_std, emp_eps, emp_eps_std,
                    pred_rho, pred_rho_std, emp_rho, emp_rho_std,
                    xlabel, ref_value, Kplot):
    colors = [plt.cm.hsv(i / max(1, Kplot)) for i in range(Kplot)]

    # --- epsilon panel (with labels) ---
    for k in range(Kplot):
        ax_eps.fill_between(
            x, pred_eps[:, k] - pred_eps_std[:, k],
            pred_eps[:, k] + pred_eps_std[:, k],
            alpha=0.25, color=colors[k][:3],
            label=f"k={k+1} prediction"
        )
        ax_eps.errorbar(
            x, emp_eps[:, k], emp_eps_std[:, k],
            linewidth=2, linestyle='', capsize=5,
            color=colors[k][:3],
    ...
    for k in range(Kplot):
        ax_rho.fill_between(
            x, pred_rho[:, k] - pred_rho_std[:, k],
               pred_rho[:, k] + pred_rho_std[:, k],
            alpha=0.25, color=colors[k][:3]
        )
        ax_rho.errorbar(
            x, emp_rho[:, k], emp_rho_std[:, k],
            linewidth=2, linestyle='', capsize=5,
            color=colors[k][:3]
        )
...
def make_figure(saved):
    ...
    # A
    subfig_A = fig.add_subfigure(top_gs[0]); axA = subfig_A.subplots(1, 1)
    if img is not None: axA.imshow(img)
    axA.set_axis_off(); add_subfigure_label(subfig_A, "A")

    # B (trials)
    subfig_B = fig.add_subfigure(top_gs[1]); axB_eps, axB_rho = subfig_B.subplots(1, 2)
    add_subfigure_label(subfig_B, "B")
    _plot_axis_pair(axB_eps, axB_rho,
        saved["trials_grid"],
        saved["pred"]["trials"]["eps"], saved["pred"]["trials"]["eps_std"],
        saved["emp"]["trials"]["eps"],  saved["emp"]["trials"]["eps_std"],
        saved["pred"]["trials"]["mean_rho"], saved["pred"]["trials"]["mean_rho_std"],
        saved["emp"]["trials"]["mean_rho"],  saved["emp"]["trials"]["mean_rho_std"],
        xlabel="Number of trials", ref_value=saved["ref_trials"], Kplot=saved["Kplot"]
    )

    # C (neurons)
    subfig_C = fig.add_subfigure(bottom_gs[0]); axC_eps, axC_rho = subfig_C.subplots(1, 2)
    add_subfigure_label(subfig_C, "C")
    _plot_axis_pair(axC_eps, axC_rho,
        saved["neurons_grid"],
        saved["pred"]["neurons"]["eps"], saved["pred"]["neurons"]["eps_std"],
        saved["emp"]["neurons"]["eps"],  saved["emp"]["neurons"]["eps_std"],
        saved["pred"]["neurons"]["mean_rho"], saved["pred"]["neurons"]["mean_rho_std"],
        saved["emp"]["neurons"]["mean_rho"],  saved["emp"]["neurons"]["mean_rho_std"],
        xlabel="Number of neurons", ref_value=saved["ref_neurons"], Kplot=saved["Kplot"]
    )

    # D (animals)
    subfig_D = fig.add_subfigure(bottom_gs[1]); axD_eps, axD_rho = subfig_D.subplots(1, 2)
    add_subfigure_label(subfig_D, "D")
    _plot_axis_pair(axD_eps, axD_rho,
        saved["D_grid"],
        saved["pred"]["animals"]["eps"], saved["pred"]["animals"]["eps_std"],
        saved["emp"]["animals"]["eps"],  saved["emp"]["animals"]["eps_std"],
        saved["pred"]["animals"]["mean_rho"], saved["pred"]["animals"]["mean_rho_std"],
        saved["emp"]["animals"]["mean_rho"],  saved["emp"]["animals"]["mean_rho_std"],
        xlabel="Number of animals", ref_value=saved["ref_animals"], Kplot=saved["Kplot"]
    )
    return fig
