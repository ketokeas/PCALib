...
def _empirical_two_groups(A_data, B_data, K, mode):
    ...
    coeffA = coeffA[:, :K]; scoreA = scoreA[:, :K]
    coeffB = coeffB[:, :K]; scoreB = scoreB[:, :K]
...
def estimate_signal_variance_std_per_pc(pots, n_steps, n_samples):
    ...
    for k in range(len(pots)):
        s = pots[k].estimate_signal_variance_uncertainty(n_steps=n_steps, n_samples=n_samples)
...
def build_grids_and_prelim(G, n_trials, N):
    ...
    return (D_grid, trials_grid, D_pre, n_pre_per_animal, pre_trials,
            neurons_grid, animals_grid, base_order, pre_ids, capacities)
...
def predict_trials_axis(pots, pre_trials, trials_grid, sigvar_std):
    K = len(pots)
    L = len(trials_grid)
    mean_rho = np.zeros((L, K)); eps = np.zeros((L, K))
    mean_rho_std = np.zeros((L, K)); eps_std = np.zeros((L, K))
    for i, tcur in enumerate(trials_grid):
        for k in range(K):
            ...
            eps_std[i, k]     = float(np.asarray(pred.get("epsilon_std", 0.0)).reshape(-1)[0])
    return {"mean_rho": mean_rho, "epsilon": eps, "mean_rho_std": mean_rho_std, "epsilon_std": eps_std}
    ...
    for i, Ncur in enumerate(neurons_grid):
        ...
        while needed > 0 and sum(len(p) for p in pools_copy) > 0:
            for a in range(D_pre):
                if needed == 0: break
                if pools_copy[a]:
                    extra.append(pools_copy[a].pop(0)); needed -= 1
...
def empirical_trials_with_reps_resample(full_data_perm, K, pre_trials, trials_grid, pre_ids,
                                        n_reps, seed):
    """
    ...
    for r in range(n_reps):
        ...
        for i, tcur in enumerate(trials_grid):
            ...
            rho_all[r, i] = rho; eps_all[r, i] = eps
    ...
    return {
        ...
        "mean_rho_all":  rho_all, "epsilon_all": eps_all
    }

def empirical_neurons_with_reps_resample(full_data_perm, G, K, pre_trials, D_pre,
                                         base_order, pre_ids, neurons_grid, n_reps, seed):
    """
...
def empirical_animals_with_reps_resample(full_data_perm, G, K, pre_trials, D_grid,
                                         n_pre_per_animal, base_order, n_reps, seed):
    """
...
def main():
    ...
    (D_grid, trials_grid, D_pre, n_pre_per_animal, pre_trials,
     neurons_grid, animals_grid, base_order, pre_ids, capacities) = build_grids_and_prelim(G, n_trials, N)

    print(f"Available data: D_pre={D_pre}, per_animal={n_pre_per_animal}, "
    ...
    print(f"Estimating K on available slice: trials={pre_trials}, neurons={len(pre_ids)}, animals={D_pre}")
    ...
    pots, _ = fit_statistics_from_dataset_diagonal(data_fit, K, G_fit, TAU_SIGMA, mode=MODE, gamma=GAMMA)
    ...
    sigvar_std = estimate_signal_variance_std_per_pc(pots, SIGVAR_N_STEPS, SIGVAR_N_SAMPLES)
    ...
    pred_neurons = predict_neurons_axis(pots, G, D_pre, base_order, pre_ids, neurons_grid, sigvar_std)

    print("Predicting | animals axis ...")
    pred_animals = predict_animals_axis(pots, G, D_grid, n_pre_per_animal, base_order, sigvar_std)
    ...
    for name, bundle in [("trials", pred_trials), ("neurons", pred_neurons), ("animals", pred_animals)]:
    ...
    emp_neurons = empirical_neurons_with_reps_resample(
        data_perm, G, K, pre_trials, D_pre, base_order, pre_ids, neurons_grid, N_REPETITIONS, seed=SEED+2000
    )
    emp_animals = empirical_animals_with_reps_resample(
        data_perm, G, K, pre_trials, D_grid, n_pre_per_animal, base_order, N_REPETITIONS, seed=SEED+3000
    )

    for name, bundle in [("trials", emp_trials), ("neurons", emp_neurons), ("animals", emp_animals)]:
