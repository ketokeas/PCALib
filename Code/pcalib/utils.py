...
def poissonize(non_convoluted_data):
    ...
    poisson_process = jax.random.poisson(jax.random.PRNGKey(key),lam=non_convoluted_data)
...
def get_empirical_accuracy_many_attempts(n_attempts,full_data,K,G,mode,size_axis,size_values):
    ...
    for attempt in range(n_attempts):
        ...
        rho_empirical[attempt,:,:], epsilon_empirical[attempt,:,:]= get_empirical_accuracy_array(full_data_shuffled,K,G,mode, size_axis, size_values)
...
def get_empirical_accuracy_array(full_data,K,G,mode,size_axis,size_values):
    ...
    if size_axis=="trials":
        ...
        if size_values[-1]>n_trials_max:
            raise Exception('Not enough trials in the dataset to use the subsampling technique!')
        ...
        for i,n_trials in enumerate(size_values):
            ...
            for k in range(K):
                ...
                epsilon_array[i,k] = 1/(2*T_eff) * np.sum((score_1[:,k]-score_2[:,k])**2)

            R_array = np.zeros([K,K])
            for k1 in range(K):
                for k2 in range(K):
                    R_array[k1,k2] = np.sqrt(np.abs(np.dot(coeff_1[:, k1], coeff_2[:, k2])))
            print("R=", R_array)
        ...
        raise ValueError('Please select one of the sizes to iterate from: "trials","neurons" or "animals"')
