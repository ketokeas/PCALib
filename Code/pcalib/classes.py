...
class Point:
    _FIELDS = ("W", "v", "L", "r", "R", "tilde_W", "U", "tilde_v", "S", "p", "tilde_R")
...
class Potential:
    def __init__(self, bar_sigma, bar_e, G, bar_xi, Z, Delta, bar_x, Xi):
        ...
        self.bar_sigma, self.bar_e, self.G = (
            jnp.array(bar_sigma),
            jnp.array(bar_e),
            jnp.array(G),
        )
        self.bar_xi, self.Z, self.Delta = (
            jnp.array(bar_xi),
            jnp.array(Z),
            jnp.array(Delta),
        )
        ...
        T = self.Z.shape[0]
        circulant_matrix = jnp.array(
            [jnp.roll(self.bar_x, t, axis=0) for t in range(T)]
        )
        self.X = jnp.einsum(
            "tsk,sr,url->kltu", circulant_matrix, self.Xi, circulant_matrix
        )
    ...
    def free_energy(self, point):
        ...
        trace_function = lambda matA, matB: jnp.trace(jnp.linalg.inv(matA) @ matB) / (
            2 * N
        )
    ...
    def gradient(self, point, ext_field=0):
        ...
        ext_field_term = jnp.tile(
            (jnp.eye(K) * ext_field)[jnp.newaxis, :, :], [D, 1, 1]
        )
        ...
        der_tilde_R += jnp.einsum("ikl,dlm,imn,dii->dkn", invA_i, p.tilde_R, eeT, G) / (
            2 * N
        )
        der_tilde_R += jnp.einsum("ikl,imn,dkn,dii->dlm", invA_i, eeT, p.tilde_R, G) / (
            2 * N
        )

        # "complicated" derivatives
        der_tilde_W += jnp.einsum(
            "ikl,imn,ink,i->lm", invA_i, invA_i, B_i, bar_sigma**2
        ) / (2 * N)
        der_U += jnp.einsum("ikl,imn,ink->lm", invA_i, invA_i, B_i) / (2 * N)
        der_tilde_v += jnp.einsum("ikl,iop,imn,ink->oplm", invA_i, eeT, invA_i, B_i) / (
            2 * N
        )
        ...
        v_X = jnp.reshape(
            jnp.moveaxis(self.tensor_contraction_with_X(p.v, X), 2, 1), (K * T, K * T)
        )
        r_X = jnp.reshape(
            jnp.moveaxis(self.tensor_contraction_with_X(p.r, X), 2, 1), (K * T, K * T)
        )
        A_2 = jnp.eye(T * K) - 2 * (W_Z + v_X)
        RxiRT = jnp.einsum("dkm,dm,dnm,ts->knts", p.R, bar_xi**2, p.R, tilde_Delta)
        RxxTRT = jnp.einsum(
            "dkl,lmts,cnm->knts", p.R + ext_field_term, xxT, p.R + ext_field_term
        )

        # InvA_2 = jnp.transpose(jnp.reshape(inv(A_2), (K, T, K, T)), (0, 2, 1, 3))
        InvA_2 = jnp.transpose(
            jnp.reshape(jnp.linalg.solve(A_2, np.eye(K * T)), (K, T, K, T)),
            (0, 2, 1, 3),
        )
        B_2 = (
            jnp.transpose(
                jnp.reshape(jnp.kron(p.L, tilde_Z) + r_X, (K, T, K, T)), (0, 2, 1, 3)
            )
            + RxiRT
            + RxxTRT
        )
        ...
        der_R += (
            jnp.einsum("kltu,dln,dn,ut->dkn", InvA_2, p.R, bar_xi**2, tilde_Delta) / N
        )
        der_R += (
            jnp.einsum("kltu,dm,ut,dkm->dlm", InvA_2, bar_xi**2, tilde_Delta, p.R) / N
        )
        der_R += (
            jnp.einsum(
                "kltu,dlm,mnut,cd->ckn",
                InvA_2,
                p.R + ext_field_term,
                xxT,
                jnp.ones([D, D]),
        ...
        # "complicated" derivatives
        der_W += jnp.einsum("kltu,ur,mnrs,nkst->lm", InvA_2, tilde_Z, InvA_2, B_2) * (
            2 / N
        )
        der_v += jnp.einsum("kltu,opur,mnrs,nkst->oplm", InvA_2, X, InvA_2, B_2) * (
            2 / N
        )
    ...
    def hessian_AB(self, point, ext_field=0):
        ...
        vX = (
            jnp.tensordot(p.v, self.X, axes=([0, 1], [0, 1]))
            .swapaxes(1, 2)
            .reshape(K * T, K * T)
        )
        rX = (
            jnp.tensordot(p.r, self.X, axes=([0, 1], [0, 1]))
            .swapaxes(1, 2)
            .reshape(K * T, K * T)
        )
        ...
        RxxT = jnp.einsum(
            "d,kmtu->dkmtu",
            jnp.ones(D),
            jnp.einsum("dkl,lmtu->kmtu", p.R + ext_field_term, xxT),
        )  # sum over d as well :)
        ...
        B = (
            ...
            + rX
            + jnp.reshape(
                jnp.einsum("dkmtu,dnm->ktnu", RxxT, p.R + ext_field_term),
                (K * T, K * T),
            )
        ...
        InvAX_tensor = jnp.tensordot(
            2 * InvA_tensor, self.X, axes=([3], [2])
        )  # Order of axes: d,d,T,d,d,T
        # Want to switch the order of the axes in InvAX_tensor: put axis 1 after the axis 4, so the new order of axes is [0,2,3,4,1,5]
        InvAX_tensor = jnp.transpose(
            InvAX_tensor, (0, 2, 3, 4, 1, 5)
        )  # Order of axes: d,T,d,d,d,T
        InvAXInvAB = jnp.tensordot(
            InvAX_tensor, InvAB_tensor, axes=([5], [2])
        )  # Order of axes: d,T,d,d,d,d,d,T
        InvAZInvAB = jnp.tensordot(
            InvAZ_tensor, InvAB_tensor, axes=([3], [2])
        )  # Order of axes: d,d,T,d,d,T
        ...
        d2W = (
            4
            * jnp.einsum(
                "abop,pq,cdqr,rs,efst,fato->bcde",
                InvA_tensor,
                tilde_Z,
                InvA_tensor,
                tilde_Z,
        ...
        d2v = (
            jnp.tensordot(InvAX_tensor, InvAXInvAB, axes=([1, 5, 0], [7, 1, 6])) / N
        )  # add the coefficient of two later because of the square of the term in the Hessian.
        # Assembling the Hessian part d^2(InvAB)/dWdv
        dWdv = jnp.tensordot(
            InvAZ_tensor, InvAXInvAB, axes=([3, 2, 0], [1, 7, 6])
        ) / N + jnp.transpose(
            jnp.tensordot(InvAX_tensor, InvAZInvAB, axes=([0, 1, 5], [4, 5, 2])) / N,
            (4, 5, 0, 1, 2, 3),
        )
        ...
        HessianAB = HessianAB.at[idx_map["W"], idx_map["W"]].set(
            jnp.reshape(d2W, [p.W.size, p.W.size])
        )
        HessianAB = HessianAB.at[idx_map["W"], idx_map["v"]].set(
            jnp.reshape(dWdv, [p.W.size, p.v.size])
        )
        HessianAB = HessianAB.at[idx_map["v"], idx_map["W"]].set(
            HessianAB[idx_map["W"], idx_map["v"]].T
        )
        HessianAB = HessianAB.at[idx_map["v"], idx_map["v"]].set(
            jnp.reshape(d2v, [p.v.size, p.v.size])
        )
        HessianAB = HessianAB.at[idx_map["v"], idx_map["v"]].set(
            HessianAB[idx_map["v"], idx_map["v"]]
            + HessianAB[idx_map["v"], idx_map["v"]].T
        )
        ...
        xi_term = jnp.einsum(
            "cklmn,cd->ckldmn", xi_term, jnp.eye(D)
        )  # Added an identity matrix in D-space.
        xxT_term = jnp.transpose(
            jnp.tensordot(InvA_tensor, xxT, axes=([2, 3], [3, 2])) / N, (1, 2, 0, 3)
        )
        xxT_term = jnp.einsum(
            "klmn,cd->ckldmn", xxT_term, jnp.ones([D, D])
        )  # Added an ones matrix in D-space, because we sum over different d.
        d2R = xi_term + xxT_term

        HessianAB = HessianAB.at[idx_map["R"], idx_map["R"]].set(
            jnp.reshape(d2R, [p.R.size, p.R.size])
        )
        HessianAB = HessianAB.at[idx_map["R"], idx_map["R"]].set(
            HessianAB[idx_map["R"], idx_map["R"]]
            + HessianAB[idx_map["R"], idx_map["R"]].T
        )

        # Now we have to assemble the cross-terms. First, the cross-term of W and L
        dWdL = jnp.transpose(
            jnp.tensordot(InvAZ_tensor, InvAZ_tensor / 2, axes=([3, 2], [2, 3])) / N,
            (1, 2, 3, 0),
        )
        HessianAB = HessianAB.at[idx_map["W"], idx_map["L"]].set(
            jnp.reshape(dWdL, [p.W.size, p.L.size])
        )
        HessianAB = HessianAB.at[idx_map["L"], idx_map["W"]].set(
            HessianAB[idx_map["W"], idx_map["L"]].T
        )

        # Next, the cross-term of W with r
        dWdr = jnp.transpose(
            jnp.tensordot(InvAZ_tensor, InvAX_tensor / 2, axes=([3, 2], [1, 5])) / N,
            (1, 2, 3, 4, 5, 0),
        )
        HessianAB = HessianAB.at[idx_map["W"], idx_map["r"]].set(
            jnp.reshape(dWdr, [p.W.size, p.r.size])
        )
        HessianAB = HessianAB.at[idx_map["r"], idx_map["W"]].set(
            HessianAB[idx_map["W"], idx_map["r"]].T
        )

        # Next, the cross-term of v with L
        dvdL = jnp.transpose(
            jnp.tensordot(InvAX_tensor, InvAZ_tensor / 2, axes=([1, 5], [3, 2])) / N,
            (1, 2, 3, 4, 5, 0),
        )
        HessianAB = HessianAB.at[idx_map["v"], idx_map["L"]].set(
            jnp.reshape(dvdL, [p.v.size, p.L.size])
        )
        HessianAB = HessianAB.at[idx_map["L"], idx_map["v"]].set(
            HessianAB[idx_map["v"], idx_map["L"]].T
        )

        # Next, the cross-tems of v with r
        dvdr = jnp.transpose(
            jnp.tensordot(InvAX_tensor, InvAX_tensor / 2, axes=([1, 5], [5, 1])) / N,
            (1, 2, 3, 4, 5, 6, 7, 0),
        )
        HessianAB = HessianAB.at[idx_map["v"], idx_map["r"]].set(
            jnp.reshape(dvdr, [p.v.size, p.r.size])
        )
        HessianAB = HessianAB.at[idx_map["r"], idx_map["v"]].set(
            HessianAB[idx_map["v"], idx_map["r"]].T
        )

        # Next, the cross-term of W with R
        # Brute force: einsum
        R_xiDelta = jnp.einsum(
            "dmk,dkl,ts,cd->cdmlts", p.R, diag_square_xi, tilde_Delta, jnp.eye(D)
        )
        RxxT = jnp.einsum(
            "dmk,klts,cd->cdmlts", p.R + ext_field_term, xxT, jnp.ones([D, D])
        )
        xiDelta_RT = jnp.einsum(
            "dkl,ts,dml,cd->cdkmts", diag_square_xi, tilde_Delta, p.R, jnp.eye(D)
        )
        xxTRT = jnp.einsum(
            "klts,dml,cd->cdkmts", xxT, p.R + ext_field_term, jnp.ones([D, D])
        )

        dWdR = (
            2
            * jnp.einsum(
                "kltu,ur,mnrs,cdokst->lmdno",
                InvA_tensor,
                tilde_Z,
                InvA_tensor,
                xxTRT + xiDelta_RT,
    ...
    def hessian_CD(self, point):
        ...
        A_i += jnp.einsum(
            "i,kl->ikl", self.bar_sigma**2, p.tilde_W
        )  # sigma^2 * tilde_W
        A_i += jnp.einsum("i,kl->ikl", jnp.ones(N), p.U)  # U
        ...
        large_sum = large_sum + jnp.transpose(
            large_sum, (0, 3, 4, 1, 2)
        )  # the other order of taking the derivative
        d2U = -1 / (2 * N) * jnp.sum(large_sum, axis=0)
        d2tildeW = (
            -1
            / (2 * N)
            * jnp.tensordot(large_sum, self.bar_sigma**4, axes=((0,), (0,)))
        )
        d2tildev = (
            -1
            / (2 * N)
            * jnp.einsum(
                "iabklmn,icd->abklcdmn",
                jnp.einsum("iklmn,iab->iabklmn", large_sum, eeT),
                eeT,
            )
        )

        # rewrite using .at[].set()
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["U"]].set(
            jnp.reshape(d2U, [p.U.size, p.U.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["tilde_W"]].set(
            jnp.reshape(d2tildeW, [p.tilde_W.size, p.tilde_W.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["tilde_v"]].set(
            jnp.reshape(d2tildev, [p.tilde_v.size, p.tilde_v.size])
        )

        dtildeWU = (
            -1
            / (2 * N)
            * jnp.tensordot(large_sum, self.bar_sigma**2, axes=((0,), (0,)))
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["U"]].set(
            jnp.reshape(dtildeWU, [p.tilde_W.size, p.U.size])
        )
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["tilde_W"]].set(
            hessian_CD[idx_map["tilde_W"], idx_map["U"]].T
        )
        ...
        dtildeWtidldev = (
            -1
            / (2 * N)
            * jnp.tensordot(tprod_temp, self.bar_sigma**2, axes=((0,), (0,)))
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["tilde_v"]].set(
            jnp.reshape(dtildeWtidldev, [p.tilde_W.size, p.tilde_v.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["tilde_W"]].set(
            hessian_CD[idx_map["tilde_W"], idx_map["tilde_v"]].T
        )

        dtildeUtidldev = (
            -1 / (2 * N) * jnp.tensordot(tprod_temp, jnp.ones(N), axes=((0,), (0,)))
        )
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["tilde_v"]].set(
            jnp.reshape(dtildeUtidldev, [p.U.size, p.tilde_v.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["U"]].set(
            hessian_CD[idx_map["U"], idx_map["tilde_v"]].T
        )
        ...
        d2tildeR = (
            ...
            * jnp.einsum(
                "dklmn,dc->dklcmn",
                jnp.einsum("iklmn,dii->dklmn", large_sum, self.G),
                jnp.eye(D),
            )
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_R"], idx_map["tilde_R"]].set(
            jnp.reshape(d2tildeR, [p.tilde_R.size, p.tilde_R.size])
        )
        ...
        dtildeWS = (
            1 / (2 * N) * jnp.einsum("ilmnk,i->lmnk", InvAInvA_i, self.bar_sigma**4)
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["S"]].set(
            jnp.reshape(dtildeWS, [p.tilde_W.size, p.S.size])
        )
        hessian_CD = hessian_CD.at[idx_map["S"], idx_map["tilde_W"]].set(
            hessian_CD[idx_map["tilde_W"], idx_map["S"]].T
        )

        dtildeUS = (
            1 / (2 * N) * jnp.einsum("ilmnk,i->lmnk", InvAInvA_i, self.bar_sigma**2)
        )
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["S"]].set(
            jnp.reshape(dtildeUS, [p.U.size, p.S.size])
        )
        hessian_CD = hessian_CD.at[idx_map["S"], idx_map["U"]].set(
            hessian_CD[idx_map["U"], idx_map["S"]].T
        )
        ...
        dtildevS = (
            ...
            * jnp.einsum(
                "ilmnk,iop->oplmnk",
                InvAInvA_i,
                jnp.einsum("iop,i->iop", eeT, self.bar_sigma**2),
            )
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["S"]].set(
            jnp.reshape(dtildevS, [p.tilde_v.size, p.S.size])
        )
        hessian_CD = hessian_CD.at[idx_map["S"], idx_map["tilde_v"]].set(
            hessian_CD[idx_map["tilde_v"], idx_map["S"]].T
        )
        ...
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["p"]].set(
            jnp.reshape(dtildeWp, [p.tilde_W.size, p.p.size])
        )
        hessian_CD = hessian_CD.at[idx_map["p"], idx_map["tilde_W"]].set(
            hessian_CD[idx_map["tilde_W"], idx_map["p"]].T
        )

        dUp = 1 / (2 * N) * jnp.einsum("ilmnk,iop->lmopnk", InvAInvA_i, eeT)
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["p"]].set(
            jnp.reshape(dUp, [p.U.size, p.p.size])
        )
        hessian_CD = hessian_CD.at[idx_map["p"], idx_map["U"]].set(
            hessian_CD[idx_map["U"], idx_map["p"]].T
        )
        ...
        dtildevp = (
            ...
            / (2 * N)
            * jnp.einsum(
                "ilmnk,ioprs->oplmrsnk",
                InvAInvA_i,
                jnp.einsum("iop,irs->ioprs", eeT, eeT),
            )
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["p"]].set(
            jnp.reshape(dtildevp, [p.tilde_v.size, p.p.size])
        )
        hessian_CD = hessian_CD.at[idx_map["p"], idx_map["tilde_v"]].set(
            hessian_CD[idx_map["tilde_v"], idx_map["p"]].T
        )
        ...
        hessian_CD = hessian_CD.at[idx_map["tilde_W"], idx_map["tilde_R"]].set(
            jnp.reshape(dtildeWtildeR, [p.tilde_W.size, p.tilde_R.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_R"], idx_map["tilde_W"]].set(
            hessian_CD[idx_map["tilde_W"], idx_map["tilde_R"]].T
        )
        ...
        hessian_CD = hessian_CD.at[idx_map["U"], idx_map["tilde_R"]].set(
            jnp.reshape(dUtildeR, [p.U.size, p.tilde_R.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_R"], idx_map["U"]].set(
            hessian_CD[idx_map["U"], idx_map["tilde_R"]].T
        )
        ...
        dtildevtildeR = (
            ...
            / (2 * N)
            * jnp.einsum(
                "ihj,iab,ice,def,ifg,dii->hjbcdag",
                eeT,
                invA_i,
                invA_i,
                p.tilde_R,
        ...
        dtildevtildeR += (
            ...
            / (2 * N)
            * jnp.einsum(
                "ihj,iab,ice,ifg,dag,dii->hjbcdef",
                eeT,
                invA_i,
                invA_i,
                eeT,
        ...
        hessian_CD = hessian_CD.at[idx_map["tilde_v"], idx_map["tilde_R"]].set(
            jnp.reshape(dtildevtildeR, [p.tilde_v.size, p.tilde_R.size])
        )
        hessian_CD = hessian_CD.at[idx_map["tilde_R"], idx_map["tilde_v"]].set(
            hessian_CD[idx_map["tilde_v"], idx_map["tilde_R"]].T
        )
    ...
    def hessian(self, point, ext_field=0):
        ...
        hessian_matrix = hessian_matrix.at[idx_map["W"], idx_map["S"]].set(
            jnp.eye(K**2) / 2
        )
        hessian_matrix = hessian_matrix.at[idx_map["S"], idx_map["W"]].set(
            jnp.eye(K**2) / 2
        )
        ...
        hessian_matrix = hessian_matrix.at[idx_map["tilde_R"], idx_map["R"]].set(
            jnp.eye(D * K**2)
        )
        hessian_matrix = hessian_matrix.at[idx_map["R"], idx_map["tilde_R"]].set(
            jnp.eye(D * K**2)
        )

        # Since E depends on tildeW and L as 1/2*trace(tildeW@L), the cross derivative is eye(K)/2
        hessian_matrix = hessian_matrix.at[idx_map["tilde_W"], idx_map["L"]].set(
            jnp.eye(K**2) / 2
        )
        hessian_matrix = hessian_matrix.at[idx_map["L"], idx_map["tilde_W"]].set(
            jnp.eye(K**2) / 2
        )

        # Since E depends on tildev and r as 1/2*sum(tildev*r), the cross derivative is 1/2*eye(K**2)
        hessian_matrix = hessian_matrix.at[idx_map["tilde_v"], idx_map["r"]].set(
            jnp.eye(K**4) / 2
        )
        hessian_matrix = hessian_matrix.at[idx_map["r"], idx_map["tilde_v"]].set(
            jnp.eye(K**4) / 2
        )

        # Since E depends on v and p as 1/2*sum(v*p), the cross derivative is 1/2*eye(K**2)
        hessian_matrix = hessian_matrix.at[idx_map["v"], idx_map["p"]].set(
            jnp.eye(K**4) / 2
        )
        hessian_matrix = hessian_matrix.at[idx_map["p"], idx_map["v"]].set(
            jnp.eye(K**4) / 2
        )
        ...
        hessian_matrix = hessian_matrix.at[:index_rangeAB, :index_rangeAB].set(
            hessian_matrix[:index_rangeAB, :index_rangeAB] + hessAB
        )

        return hessian_matrix

    def find_saddle_point(
        self, init_point, gamma=1, n_steps_max=20, eps=1e-5, ext_field=0
    ):
        """
    ...
    def find_solution(self, eps=10 ** (-4), verbose=False):
        ...
        ext_field = -(10**-3)
        init_point, converged = copied_potential.find_saddle_point(
            init_point, ext_field=ext_field
        )
        ...
        if verbose:
            ...
            with tqdm(total=100) as pbar:
                while (
                    already_added < 1
                ):  # and attempted_step_size > 1e-4: # Either we have added all the xi or the step size is too small
                    ...
                    copied_potential = Potential(
                        ...
                        self.G,
                        saved_bar_xi
                        * jnp.minimum(1, already_added + attempted_step_size),
                        self.Z,
                        self.Delta,
                        self.bar_x,
                        saved_Xi * 0,
                    )
                    point, converged = copied_potential.find_saddle_point(
                        init_point, ext_field=ext_field
                    )
                    R_diag_elements = jnp.einsum("dii->di", point.R) + np.sqrt(eps)
                    R_diag_elements_old = jnp.einsum("dii->di", init_point.R) + np.sqrt(
                        eps
                    )
                    if not converged:
            ...
            while (
                ...
                copied_potential = Potential(
                    ...
                    self.bar_e,
                    self.G,
                    saved_bar_xi * jnp.minimum(1, already_added + attempted_step_size),
                    self.Z,
                    self.Delta,
                    self.bar_x,
                    saved_Xi * 0,
        ...
        if verbose:
            ...
            already_added = 0.0
            maximal_attempted_step_size = np.min(
                [
                    (np.min(np.std(self.bar_x, axis=0)) / 100) / np.max(saved_bar_sigma)
                    + eps,
                    1,
                ]
            )
            ...
            with tqdm(total=100) as pbar:
                while already_added < 1:  # and attempted_step_size > 1e-4:
                    ...
                    if not converged:
                        ...
                        jnp.min(R_diag_elements / R_diag_elements_old) < 0.5
                    ):  # If the new R has a diagonal element that is less than half of the old one, we stop
                        attempted_step_size /= 2
                    else:
                        already_added += attempted_step_size
                        attempted_step_size *= 2
                        attempted_step_size = np.min(
                            [attempted_step_size, maximal_attempted_step_size]
                        )
                        init_point = point
                        pbar.update(100 * already_added - pbar.n)
        ...
        init_point.tilde_v = np.einsum(
            "klmn,am,nb->klab", init_point.tilde_v, c_matrix, c_matrix
        )
    ...
    def generate_sample_data(self, n_samples=10, same_delta_F=False):
        ...
        if jnp.max(jnp.abs(self.Xi)) < T ** (-3):
            ...
            delta_F = jax.random.multivariate_normal(
                k1, jnp.zeros(T), self.Xi * N, shape=(N,), method="svd"
            ).T
            delta_Fs = jnp.tile(delta_F[None, :, :], (n_samples, 1, 1))
        else:
            delta_Fs = jax.random.multivariate_normal(
                k1, jnp.zeros(T), self.Xi * N, shape=(n_samples, N), method="svd"
            )
        ...
        calcium_part = jnp.fft.ifft(
            jnp.einsum("sli,tk->slk", fft_delta_F, fft_bar_x), axis=1
        ).real  # [samples, T, K]
        calcium_part = jnp.einsum(
            "slk,ik->sli", calcium_part, self.bar_e
        )  # [samples, T, N]
        ...
        delta_x = jnp.transpose(
            jax.random.multivariate_normal(
                k2, jnp.zeros(T), self.Delta, shape=(n_samples, K, D), method="svd"
            ),
            (0, 3, 1, 2),
        )  # [samples, T, K, D]

        delta_x = jnp.einsum("stkd,dk->stkd", delta_x, self.bar_xi)  # scale
        trial_var = jnp.einsum(
            "stkd,dii,ik->sti", delta_x, self.G, self.bar_e
        )  # [samples, T, N]

        # Noise
        key, k3 = jax.random.split(key)
        noise = jax.random.multivariate_normal(
            k3, jnp.zeros(T), self.Z, shape=(n_samples, N), method="svd"
        )
    ...
    def rho(self, point):
        ...
        matrices_to_invert = jnp.linalg.inv(
            jnp.einsum("i,kl->ikl", self.bar_sigma**2, point.tilde_W)
            + point.U[jnp.newaxis, :, :]
            + veeT
        )
        ...
        term_1 = (
            ...
            * jnp.einsum(
                "ikl,ilm,imn->ikn",
                matrices_to_invert,
                jnp.einsum("i,kl->ikl", self.bar_sigma**2, point.S)
                + jnp.einsum("klmn,ikl->imn", point.p, eeT),
                matrices_to_invert,
            )
            + 1 / (2 * N) * eeT
        )

        intermediate_prod = jnp.einsum(
            "ikl,dii,dlm->ikm", matrices_to_invert, self.G, point.tilde_R
        )
        term_2 = (
            1
            / (2 * N)
            * (
                jnp.einsum("ikl,ilm->ikm", intermediate_prod, eeT)
                + jnp.einsum("ikl,iml->ikm", eeT, intermediate_prod)
            )
        )
        term_3 = (
            1
            / (2 * N)
            * jnp.einsum("ikl,ilm,inm->ikn", intermediate_prod, eeT, intermediate_prod)
        )
    ...
    def epsilon(self, point):
        ...
        RxiRT = jnp.einsum(
            "dkm,dm,dnm,ts->knts", p.R, self.bar_xi**2, p.R, tilde_Delta
        )
        RxxTRT = jnp.einsum("dkl,lmts,cnm->knts", p.R, xxT, p.R)
        W_Z = jnp.kron(p.W, tilde_Z)
        v_X = jnp.reshape(
            jnp.moveaxis(jnp.einsum("klmn,klts->mnts", p.v, self.X), 2, 1),
            (K * T, K * T),
        )
        A_2 = jnp.eye(T * K) - 2 * (W_Z + v_X)
        InvA_2 = jnp.transpose(jnp.reshape(inv(A_2), (K, T, K, T)), (0, 2, 1, 3))
        ...
        B_no_xxT = (
            jnp.einsum("kl,ts->klts", p.L, tilde_Z)
            + jnp.einsum("klmn,klts->mnts", p.r, self.X)
            + RxiRT
        )
        ...
        pt1 = jnp.einsum("kl,ts->klts", p.W, tilde_Z) + jnp.einsum(
            "klmn,klts->mnts", p.v, self.X
        )
        Fourth_term = 2 * jnp.einsum(
            "klts,lmsu,mnuv,vw,nowt->ko", pt1, InvA_2, B_no_xxT, Id_minus_Ones, InvA_2
        )
    ...
    def estimate_theory_error_bars_from_signal_variance(
        self, point, signal_std, delta=0.05
    ):
        """
        ...
        rho_std = np.sqrt(
            np.sum(
                (grad_rho_i * signal_std[np.newaxis, np.newaxis, np.newaxis, :]) ** 2,
                axis=3,
            )
        )
    ...
    def top_eigenvalues(self, point):
        ...
        RxiRT = jnp.einsum(
            "dkl,dlm,dnm,ts->knts", p.R, diag_bar_xi_squared, p.R, tilde_Delta
        )
        RxxTRT = jnp.einsum("dkl,lmts,cnm->knts", p.R, xxT, p.R)
        W_Z = jnp.kron(p.W, tilde_Z)
        v_X = jnp.reshape(
            jnp.moveaxis(self.tensor_contraction_with_X(p.v, self.X), 2, 1),
            (K * T, K * T),
        )
        r_X = jnp.reshape(
            jnp.moveaxis(self.tensor_contraction_with_X(p.r, self.X), 2, 1),
            (K * T, K * T),
        )
        A_2 = jnp.eye(T * K) - 2 * (W_Z + v_X)
        InvA_2 = jnp.transpose(jnp.reshape(inv(A_2), (K, T, K, T)), (0, 2, 1, 3))
    ...
    def estimate_signal_variance_uncertainty(self, n_steps=30, n_samples=100):
        ...
        for sample in range(n_samples):
            ...
            lambdas[sample, :] = (
                eigs[:K] / N
            )  # dividing by N due to normalization of vectors as |e|^2=N.
        ...
        for k in range(K):
            ...
            derivative_matrix[:, k] = (lambdas_new - lambdas_mean) / (
                0.05 * signal_variances_current[k]
            )
...
jax.tree_util.register_pytree_node(
    Potential, Potential.tree_flatten, Potential.tree_unflatten
)
